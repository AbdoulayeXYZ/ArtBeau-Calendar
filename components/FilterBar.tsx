...
export default function FilterBar({
    ...
    return (
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="bg-white p-1.5 lg:p-2 rounded-2xl lg:rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-100 flex items-center gap-1 w-full md:w-auto overflow-x-auto no-scrollbar scroll-smooth">
                {periods.map((p) => (
                    <button
                        key={p.id}
                    ...
                    >
                        {period === p.id && (
                            <motion.div
                                ...
                                className="absolute inset-0 bg-slate-900 rounded-xl lg:rounded-[1.6rem] shadow-lg shadow-slate-900/20"
                                transition={{ type: "spring", stiffness: 300, damping: 30 }}
                            />
                        )}
                        <p.icon className={cn("w-3 h-3 lg:w-4 lg:h-4 relative z-10", period === p.id ? "text-primary" : "text-slate-300")} />
            ...
            <div className="flex items-center gap-2 lg:gap-4 w-full md:w-auto overflow-x-auto no-scrollbar scroll-smooth pb-1 md:pb-0">
                ...
                >
                    <Clock className={cn("w-3 h-3 lg:w-4 lg:h-4", availableNowOnly ? "text-white" : "text-slate-300")} />
                    Maintenant
                </button>
                ...
                >
                    <Building2 className={cn("w-3 h-3 lg:w-4 lg:h-4", logeBgOnly ? "text-white" : "text-slate-300")} />
                    Dortoir
                </button>

                <div className="h-8 w-px bg-slate-200 hidden xl:block mx-1" />

                <div className="bg-slate-50 px-5 lg:px-6 py-3 lg:py-4 rounded-xl lg:rounded-[2rem] border border-slate-100 flex items-center gap-2 w-full md:w-auto min-w-max">
                    <Filter className="w-3 h-3 lg:w-4 lg:h-4 text-slate-300" />
                    <span className="text-[8px] lg:text-[10px] font-black text-slate-400 uppercase tracking-widest">Filtres</span>
                </div>
            </div>
